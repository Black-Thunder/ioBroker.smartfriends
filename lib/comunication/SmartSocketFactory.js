...
class SmartSocketFactory {
	static createSocketAndLogin(that, ipAddress, port, caFileName, username, password, cSymbol, shcVersion, shApiVersion, dataHandler, startKeepAlive) {
		const returnPromise = new Promise((resolve, reject) => {
			if (dataHandler) {
				this.createSocket(that, ipAddress, port, caFileName, dataHandler)
					.catch(reason => {
						reject(reason);
					})
					.then(socket => {
	...
	static createSocket(that, ipAddress, port, caFileName, dataHandler) {
		const caText = fs.readFileSync(__dirname + "/" + caFileName, "utf8");
		const socket = new SmartSocket.SmartSocket(that, ipAddress, port, caText);
		const returnPromise = new Promise((resolve, reject) => {

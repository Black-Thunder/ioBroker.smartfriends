...
class SmartSocket {
			...
			this.internalSocket = tls.connect(socketOptions, () => {
				if (this.internalSocket) {
					...
					if (!this.internalSocket.authorized) {
						gthat.log.error("Setup connection not authorized with error: " + this.internalSocket.authorizationError);
	...
	setupSocketEvents() {
		...
		if (this.internalSocket) {
			this.internalSocket.on("data", (data) => {
				if (data.indexOf("\n") < 0) {
					...
					for (const tempSplitString of tempSplit) {
						if (tempSplitString !== "") {
							gthat.log.silly("Received data: " + tempSplitString);
							if (this.dataDelegate) {
			...
			this.internalSocket.on("timeout", () => {
				gthat.log.debug("Socket timed out");
				setTimeout(() => {
					if (this.dataDelegate) {
						this.dataDelegate.Connect();
					}
				}, 1000);
			});
			this.internalSocket.on("end", () => {
				gthat.log.debug("Socket ended");
				setImmediate(() => {
					if (this.dataDelegate) {
						this.dataDelegate.handleDisconnect();
					}
	...
	sendJSONCommand(command) {
		...
		const localPromise = new Deffered.default((resolve, reject) => {
			if (this.internalSocket && command) {
				gthat.log.silly("Send JSON: " + command.toString());
	...
	sendAndRecieveCommand(command, sessionkey) {
		const localPromise = new Deffered.default((resolve, reject) => {
			if (this.internalSocket && command) {
				gthat.log.silly("Send and recieve JSON: " + command.toString(sessionkey));

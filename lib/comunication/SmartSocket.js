...
class SmartSocket {
	...
	setupSocket(dataDelegate) {
		...
		const socketOptions = {
			...
			port: this.port,
			ca: this.certText,
			rejectUnauthorized: this.adapter.config.ignoreSslErrors ? false : true,
			...
			this.internalSocket = tls
				.connect(socketOptions, () => {
					if (this.internalSocket) {
						...
						if (!this.internalSocket.authorized && !this.adapter.config.ignoreSslErrors) {
						...
						} else if (this.internalSocket.authorized || this.adapter.config.ignoreSslErrors) {

...
class SmartSocket {
	constructor(adapter, ipAddress, port, certText, ignoreSslErrors) {
	...
	setupSocket(dataDelegate) {
		...
		// Only validate server identity if SSL errors are NOT ignored
		if (!this.ignoreSslErrors) {
			socketOptions.checkServerIdentity = (host, cert) => {
			...
			this.internalSocket = tls
				.connect(socketOptions, () => {
					if (this.internalSocket) {
						...
						if (!this.internalSocket.authorized && !this.ignoreSslErrors) {
						...
						} else if (this.internalSocket.authorized || this.ignoreSslErrors) {
							this.internalSocket.setEncoding("utf8");
							if (this.ignoreSslErrors && !this.internalSocket.authorized) {
								this.adapter.log.warn(
									`SSL certificate validation disabled. Connection established despite certificate error: ${this.internalSocket.authorizationError}`,
								);
							}

...
class SchellenbergBridge {
	...
	async Connect() {
		...
		try {
			...
			this.connectionStatus = commonDefines.ConnectionStatus.DISCONNECTED;
	...
	handleDisconnect(shouldReconnect = true) {
		if (this.connectionStatus === commonDefines.ConnectionStatus.DISCONNECTED) {
			return;
		}
		this.connectionStatus = commonDefines.ConnectionStatus.DISCONNECTED;
	...
	handleSpecificResponseCode(parsedResponse, codeNumber) {
		...
		const isIncompatibleMessage = message.includes(
			"die verwendete software ist mit dieser smart friends box nicht kompatibel",
		);
		const isInformationalPopup = message.includes("folgende lizenzen") || message.includes("update");

		switch (codeNumber) {
			case 5: {
				// Ignore update messages and warnings when licenses are expiring soon
				// Messages regarding incompatible software versions are already handled with response code -1
				if (isInformationalPopup || isIncompatibleMessage) {
					return;
				}
	...
	reconnect() {
		if (
			this.connectionStatus === commonDefines.ConnectionStatus.CONNECTING ||
			this.connectionStatus === commonDefines.ConnectionStatus.RECONNECT_SCHEDULED
		) {
			return;
		}
		...
		this.adapter.log.warn(
			this.retryCounter <= maxRetries
				...
				: `Connection to gateway lost, connection temporarily disabled! Trying again in ${delay / 60000} minutes.`,
		);

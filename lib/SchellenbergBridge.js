...
class SchellenbergBridge {
	...
	handleResponseCode(parsedResponse) {
		...
		const responseHandlers = {
			[-1]: r => this.handleSpecificResponseCode(r, -1),
			[1]: r => {
				this.retryCounter = 0;
	...
	handleSpecificResponseCode(parsedResponse, codeNumber) {
		...
		switch (codeNumber) {
			case -1: {
				// Disconnect because of incompatible software version
				if (isIncompatibleMessage) {
					this.adapter.log.error(
						"Connection was rejected by the gateway due to incompatible software version. Please check your connection parameters in the adapter settings.",
					);
				}

				// Reject all messages with response code -1, as they indicate an error

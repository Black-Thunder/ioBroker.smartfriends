...
class SchellenbergBridge {
	...
	async Connect() {
		SmartSocketFactory.default.createSocketAndLogin(gthat, this.ip, this.port, "CA.pem", this.username, this.password, this.cSymbol, this.shcVersion, this.shApiVersion, this, true)
			...
			.catch(err => {
				gthat.log.error(`Connecting to gateway failed (error code: ${err.response.errorCode}): ${err.responseMessage}`);
			...
			.then(response => {
				...
				gthat.log.error(`Processing device infos failed: ${JSON.stringify(err)}`);
			});
	}
	...
	async processAllNewDeviceInfos(response) {
		...
		for (let i = 0; i < newDeviceInfos.length; i++) {
			...
			if (device.deviceTypClient.includes(commonDefines.KnownDeviceTypes.AwningEngine.type)) {
			...
			switch (parsedResponse.responseCode) {
				...
				case 5:
					...
					if (parsedResponse.responseMessage.toLowerCase().includes("folgende lizenzen")) {
						gthat.log.warn("Licenses are expiring soon. Check official app for details. Can't continue!");
						this.handleDisconnect();
						break;
					}
				...
				case 203: // Ignore as it's just an status message for RemoteHome connection (official cloud service)
					break;
				default:
					gthat.log.warn(`Unknown message response code: ${parsedResponse.responseCode} (message: '${parsedResponse.responseMessage}') - Please report this to the developer!`);
					break;
			}
		}
	}

...
class SchellenbergBridge {
    ...
    async Connect(callback) {
        ...
        SmartSocketFactory.default.createSocketAndLogin(this.ip, this.port, "./lib/comunication/CA.pem", this.username, this.password, "D19033i", "2.15.0", "2.15", this, true)
        .then(returnedSocket => {
    ...
    handleMessage(data) {
        if (data) {
            let parsedResponse = JSONResponse.default.fromJSONString(data);
    ...
    handleUpdate(response) {
        if (response.responseMessage) {
            switch (response.responseMessage) {
                case "newDeviceValue":
                    let responseNewDeviceValue = this.handleNewDeviceValue(DeviceValue.default.fromObject(response.response));
                    break;
                case "newDeviceInfo":
                    break;
                default:

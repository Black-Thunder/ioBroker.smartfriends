...
class SchellenbergBridge {
	...
	handleResponseCode(parsedResponse) {
		// Map with all response code handlers
	...
	handleSpecificResponseCode(parsedResponse, codeNumber) {
		...
		const isParalellLoginMessage = message.includes("mit dem gleichen login angemeldet");

		switch (codeNumber) {
			case 5: {
				// Ignore all other messages and warnings with the same response code
				// e.g. { "response" : { "buttons" : [ { "buttonKind" : "ok", "buttonPosition" : 1, "buttonRequestID" : 1, "buttonText" : "Ok", "validate" : false } ], "nextCommand" : "success" }, "responseCode" : 5, "responseMessage" : "Beim Verarbeiten der Lizenz ist ein Fehler aufgetreten. Bitte starten Sie den HomeServer neu. Sollte der Fehler weiter bestehen bleiben, wenden Sie sich bitte an den Support. Fehlercode (2501)" }
				if (!isParalellLoginMessage) {
					return;
				}

				// Disconnect because of login on another device or with the official app
				// { "response" : { "buttons" : [ { "buttonKind" : "ok", "buttonPosition" : 1, "buttonRequestID" : 1, "buttonText" : "Ok", "validate" : false } ], "nextCommand" : "success" }, "responseCode" : 5, "responseMessage" : "Sie wurden abgemeldet, weil sich gerade jemand mit dem gleichen Login angemeldet hat." }
				this.adapter.log.warn(
	...
	handleErrorResponse(parsedResponse) {
		...
		switch (errorCode) {
			...
			case 70:
				this.adapter.log.error(
					"Login rejected by the gateway (errorCode 70). Please verify the configured connection parameters, especially the configured CSymbol (usually includes a trailing letter).",
				);
				break;

			default:
				this.adapter.log.error(
					`Gateway returned error (responseCode -1, errorCode ${errorCode}): ${parsedResponse.responseMessage}`,
				);
				break;
		}

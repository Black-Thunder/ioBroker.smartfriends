...
class SchellenbergBridge {
	...
	async processAllNewDeviceInfos(response) {
		try {
			...
			const compatibilityConfigurationVersion =
				response.newCompatibilityConfiguration.compatibilityConfigurationVersion;
			let knownDevices = 0;

			gthat.log.debug(
				`Received all devices:\r\n${JSON.stringify(newDeviceInfos)}\r\n-------\r\n${JSON.stringify(compatibilityConfigurationVersion)}`,
			);

			const ignoredDeviceTypes = [
				"${AstroFunction}",
				"${Static_PC_Controller}",
				"${SystemFunctions}",
			...
			for (const device of response.newDeviceInfos.values) {
				if (ignoredDeviceTypes.includes(device.deviceTypClient)) {
					gthat.log.debug(
						`Ignoring device with type '${device.deviceTypClient}' (Device ID: ${device.deviceID}) as it's not supported.`,
					);
					continue;
				}
				...
				if (
					...
					gthat.log.warn(
						`Unsupported device type received: ${device.deviceTypClient} (Device ID: ${device.deviceID}) - Please report this to the developer!`,
					);
				}
			}

			gthat.log.info(
				`Processed ${newDeviceInfos.length} device(s) and added ${knownDevices} device(s). Adapter successfully started.`,
			);
		} catch (error) {

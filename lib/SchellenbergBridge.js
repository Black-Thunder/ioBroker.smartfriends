		...
		if (data) {
			...
			switch (parsedResponse.responseCode) {
				...
				case 5:
					...
					gthat.log.warn("Connection was closed, e.g. because credentials were used to login on a different device or the official app. Reconnecting (try " + this.retryCounter + " of " + maxRetries + ")...");
					this.reconnect();
					break;

				case 84:
					gthat.setAdapterConnectionState(false);
					gthat.log.warn("The TLS/SSL connection has been closed. Reconnecting (try " + this.retryCounter + " of " + maxRetries + ")...");
					this.reconnect();
					break;
